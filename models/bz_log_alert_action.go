...
type BzLogAlertAction struct {
	...
	AlertId     string    `xorm:"not null default '' index(idx_alert) VARCHAR(50)"`
	AlertName   string    `xorm:"index(idx_alert) VARCHAR(100)"`
	MonitorId   string    `xorm:"index(idx_monitor) VARCHAR(50)"`
	MonitorName string    `xorm:"index(idx_monitor) VARCHAR(100)"`
